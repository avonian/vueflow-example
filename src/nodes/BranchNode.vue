...
const label = computed(() => {
  switch (props.data.condition) {
    case 'opened_any_email':
      return 'opened any email'
    case 'opened_specific_email':
      return 'opened a specific email'
    case 'clicked_any_link':
      return 'clicked any link'
    case 'clicked_specific_link':
      return 'clicked a specific link'
  }
  return ''
})
</script>
...
<template>
  ...
  >
      ...
      <div class="px-3 py-3 flex items-center">
        <div class="flex items-center justify-between w-full">
          <div class="flex items-center space-x-2">
            ...
            >
              <Icon name="branch" class-list="fill-gray-400" />
            </div>
            <div class="flex flex-col">
              ...
              <span class="italic text-sm text-gray-500">within {{ data.timer_value }} {{ data.timer_unit }}</span>
            </div>
          </div>
    ...
    <Handle id="source-false" type="source" :connectable="false" :position="Position.Right" />
    <div
